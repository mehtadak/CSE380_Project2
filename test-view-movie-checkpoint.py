...
try:
	...
	VEIWPARAMS = {'title': 'True', "synopsis": 'True', 'genre': 'True', 'critic': 'True', 'audience': 'True', 'reviews': 'True'}
	...
	solution = json.dumps({'status': 1, 'data': {'title': 'The Conjuring', 'synopsis': 'Paranormal investigators Ed and Lorraine Warren work to help a family terrorized by a dark presence in their farmhouse', 'genre': ['Childrens'], 'critic': '0.00', 'audience': '1.00', 'reviews': [{'user': 'karen', 'rating': '1','text': 'Who said this was a kids movie?? My little darling is traumatized!!'}]}})
	...
	for x in solution_dict['data']:
		if x == 'genre':
			if len(solution_dict['data'][x]) != len(view_data['data'][x]):
				...
				for genre in view_data['data'][x]:
					if genre not in solution_dict['data'][x]:
						quit()
		elif x == 'reviews':
			if len(solution_dict['data'][x]) != len(view_data['data'][x]):
				quit()
			else:
				for review in view_data['data'][x]:
					match = False
					for sol_review in solution_dict['data'][x]:
